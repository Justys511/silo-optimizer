...
function ChartTooltip({ active, payload, label, unit }: {
  ...
  return (
    <div style={{ background: "#111827", border: "1px solid #374151", borderRadius: 6,
                  padding: "8px 10px", fontSize: 12, lineHeight: "20px" }}>
...
function ImprovementBadge({ label, pct, lowerIsBetter = false }: {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", alignItems: "center", minWidth: 80 }}>
      ...
      <div style={{ color, fontWeight: 700, fontSize: 18, fontFamily: "monospace" }}>
        {arrow} {Math.abs(pct)}%
      </div>
    </div>
  );
  ...
  return (
    <div style={{
      ...
      <div style={{ display: "flex", alignItems: "center", marginBottom: 24 }}>
        <div>
          <div style={{ fontSize: 18, fontWeight: 700, color: "#f9fafb" }}>Algorithm Comparison</div>
            ...
            <div key={entry.metric} style={{ background: "#1f2937", borderRadius: 12, padding: "16px 20px" }}>
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 10 }}>
                <span style={{ fontSize: 13, fontWeight: 600, color: "#d1d5db" }}>
                  {entry.metric}
                  <span style={{ color: "#6b7280", fontWeight: 400, marginLeft: 6, fontSize: 11 }}>{entry.unit}</span>
                </span>
                <span style={{ fontSize: 10, fontWeight: 500, color: hint.color,
                               background: hint.color + "18", borderRadius: 4, padding: "2px 8px" }}>
                  {hint.label}
                </span>
              </div>
              ...
              <ResponsiveContainer width="100%" height={180}>
                <BarChart data={[entry]} margin={{ top: 4, right: 4, left: -10, bottom: 0 }}>
                  ...
                  <XAxis hide />
                  <YAxis domain={[yMin, yMax]} tick={{ fill: "#6b7280", fontSize: 10 }} width={40} />
        ...
        <div style={{ display: "flex", gap: 16, marginTop: 20, flexWrap: "wrap" }}>
          {isn && (
            <div style={{ background: "#1f2937", borderRadius: 12, padding: "14px 24px", flex: 1 }}>
              <div style={{ color: "#9ca3af", fontSize: 11, marginBottom: 10 }}>Smart vs Naive improvement</div>
          ...
          {ios && (
            <div style={{ background: "#1f2937", borderRadius: 12, padding: "14px 24px", flex: 1,
              ...
              <div style={{ color: "#22c55e", fontSize: 11, fontWeight: 600, marginBottom: 10 }}>
